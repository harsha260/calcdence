...
class AuthProvider extends ChangeNotifier {
  ...
  Future<void> _doInitialize() async {
    ...
    if (hasCredentials) {
      ...
      debugPrint('AuthProvider: Attempting silent login for $username');

      if (username != null && password != null) {
        _username = username;
        final result = await _apiService.login(username, password);
        debugPrint('AuthProvider: Silent login result: ${result['success']}');

        if (result['success']) {
          // Save session key
          await _storageService.saveSessionKey(result['sessionKey']);
          _state = AuthState.authenticated;
          debugPrint('AuthProvider: Initialization complete - authenticated');
          return;
        }
      }
    }
    ...
    debugPrint('AuthProvider: Initialization complete - unauthenticated');
    _state = AuthState.unauthenticated;
  }
  ...
  Future<bool> login(String username, String password) async {
    ...
    try {
      ...
      if (result['success']) {
        ...
        _username = username;
        _state = AuthState.authenticated;
        debugPrint('AuthProvider: Login successful, state = authenticated');
        notifyListeners();
        return true;
      } else {
